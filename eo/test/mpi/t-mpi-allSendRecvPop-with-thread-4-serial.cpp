...
void sendThread( eoGenContinue<EOT>& cont, std::atomic< size_t >& count )
{
    ...
    while ( !em.empty() )
	{
	    ...
	    Node::comm().send(dst, Channel::Commands, pop);
	    int msg;
	    Node::comm().recv(dst, Channel::Messages, msg);
	    em.pop();
...
void recvThread( eoGenContinue<EOT>& cont, std::atomic< size_t >& count )
{
    ...
    while ( count > 0 ) // works with "if" instead "while" without waiting for recipies
	{
	    eo::log << eo::quiet << "r" << count;
	    eo::log.flush();

	    mpi::status stat = Node::comm().probe(mpi::any_source, Channel::Commands);

	    eoPop<EOT> pop;
	    Node::comm().recv(stat.source(), Channel::Commands, pop);
	    Node::comm().send(stat.source(), Channel::Messages, Message::Continue);
...
void sendRecvThread( eoGenContinue<EOT>& cont, std::atomic< size_t >& count )
{
    while ( cont.thisGeneration < cont.totalGenerations() )
	{
	    sendThread( cont, count );
	    recvThread( cont, count );
	}
}
...
int main(int ac, char** av)
{
    ...
    std::thread t4(sendRecvThread, std::ref(cont), std::ref(count));
