...
int main(int argc, char *argv[])
{
    ...
    vector< vector<int> > to_data(to.size(), vector<int>(5, 42) );
    ...
    for (size_t i = 0; i < to.size(); ++i)
    	{
	    reqs.push_back( topology.comm().Isend(to_data[i].data(), to_data[i].size(), INT, to[i], 0) );
    	}

    for (size_t i = 0; i < from.size(); ++i)
    	{
    	    reqs.push_back( topology.comm().Irecv(from_data[i].data(), from_data[i].size(), INT, from[i], 0) );
    	}

    Request::Waitall(reqs.size(), reqs.data());
