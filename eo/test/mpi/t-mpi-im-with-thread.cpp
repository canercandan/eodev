...
public:
	...
	while (/*!toStop*/1)
		...
		if ( !em.empty() )
			...
			// if (!pop.empty())
			//     {
			// Node::comm().send( dst, 0, true ); // it works
			Node::comm().send( dst, 0, pop );
			// }
			// else
			//     {
			// 	Node::comm().send( dst, 0, false ); // doesnot works
			//     }
	...
	while (/*!toStop*/1)
		...
		//     {
		// bool isValid = false;
		// Node::comm().recv( mpi::any_source, 0, isValid );
		// std::cout << "v(" << isValid << ") ";
		// std::cout.flush();
		// if ( isValid )
		//     {
			eoPop<EOT> pop;
			Node::comm().recv( mpi::any_source, 0, pop );
			imm.push(pop);
		    // }
		// }
		// imm_mutex.unlock();
...
{
    Node::init( ac, av, MPI_THREAD_MULTIPLE/*MPI_THREAD_SERIALIZED*/ );
    assert( MPI::Query_thread() == MPI_THREAD_MULTIPLE );
