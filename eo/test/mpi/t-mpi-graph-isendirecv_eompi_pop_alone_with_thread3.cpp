...
void recvThread( eoGenContinue<EOT>& cont, eoPop<EOT>& pop, vector< double >& vecProba, mpi::communicator& topology, vector<int>& to, vector<int>& from )
{
...
int main(int argc, char *argv[])
{
    ...
    thread t2( recvThread, ref(cont), ref(pop), ref(vecProba), ref(topology), ref(to), ref(from) );
