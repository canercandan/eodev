...
#ifndef _my_make_checkpoint_h
#define _my_make_checkpoint_h
...
eoCheckPoint<EOT>& do_make_checkpoint(eoParser& _parser, eoState& _state, eoValueParam<unsigned long>& _eval, eoContinue<EOT>& _continue)
{
    ...
    //---------------------------
    eoValueParam<bool>& printBestParam = _parser.getORcreateParam(false, "printBestStat", "Print Best/avg/stdev every gen.", '\0', "Output");
    eoValueParam<bool>& plotBestParam = _parser.getORcreateParam(false, "plotBestStat", "Plot Best/avg Stat", '\0', "Output - Graphical");
    eoValueParam<bool>& fileBestParam = _parser.getORcreateParam(true, "fileBestStat", "Output bes/avg/std to file", '\0', "Output - Disk");
